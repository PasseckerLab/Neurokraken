...
class _Devices:
    ...
    def rotary_encoder(self, pins:tuple[int,int], logging=True, controls=False,
        ...
        serial_out = {'reset_wheel': rotary_encoder(pint=[3,4], controls=True)} # optional
        print(get.read_in('wheel_pos')) # -5738
        get.send_out('reset_wheel', 1) # reset the wheel position to 0 (only if control provided to serial_out)
        """
        if not controls:
    ...
    def _time_micros(self, logging=False):
        ...
        is sampling every 1 millisecond but can be used for debugging purposes with the direct
        networker mode.
        Application example:
    ...
    def direct_on(self, pin:int, start_value=False, default_value=False):
